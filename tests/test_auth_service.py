...
def test_register_user_creates_new_user(db_session: Session):
    data = UserCreate(
...
def test_register_user_raises_when_email_exists(db_session: Session):
    data = UserCreate(
...
def test_login_user_returns_token(db_session: Session):
    data = UserCreate(
